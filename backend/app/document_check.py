"""文件型態確認:上傳的內容是不是它自稱要放的那個槽位(訴願書/送達證書/原處分書/訴願答辯書)。
...
_SLOT_KEYWORDS: dict[DocumentSlot, dict[str, list[str]]] = {
    ...
    "answer": {
        "strong": ["答辯書"],
        # 答辯書必然引述它所答辯的那份裁處書,原處分書的特徵會同時高分;這裡的輔助特徵
        # 全是答辯文書獨有的段名與結尾套語,用來把分數壓過原處分書
        "supporting": [
            "答辯聲明",
            "程序答辯",
            "實體答辯",
...
_GEMINI_PROMPT = (
    "以下是一份行政訴願案件卷宗中的文件原文,請判斷它屬於「訴願書」「送達證書」「原處分書」「訴願答辯書」"
    "四者之一,或者四者都不是。訴願書由訴願人提出、訴願答辯書由原處分機關提出,兩者都會引述同一份原處分,"
    "以「誰在陳述」分辨。只依據原文內容判斷,證據不足就回答「無法確認」,不要猜測。\n"
